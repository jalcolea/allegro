...
{
    public class VolatilityCE  : UiClassEvent
    {
        ...
        /* Correlation from archive UI
        Message from archive UI */
        public UiEventResult BeforeRetrieveData_911(object sender, RetrieveDataEventArgs e)
        {
            _view.DataSource.Tables["correlation"].ExtendedProperties.Remove("From Archive");
            if (ToolbarHelper.CheckStateButton(_view, "From Archive"))
            {
                _view.DataSource.Tables["correlation"].ExtendedProperties["From Archive"] = "correlation";
            }
            return new UiEventResult(EventStatus.Continue);
        }
        /* Correlation data from archive button
        Message data from archive button */
        public UiEventResult InitView_911()
        {
            if (_view.Name.Equals("Correlation"))
            {
